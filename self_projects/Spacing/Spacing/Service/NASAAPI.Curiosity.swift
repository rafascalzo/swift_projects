...
extension NASAAPI {
    ...
    struct Curiosity {
        ...
        static func queryByEarthDate(rover: RoverName, queryParameters: MarsEarthDateQueryingParameters, completion: @escaping MarsRoversCompletion) {
            ...
            case .opportunity:
                url = URLComposer.buildURL(baseUrl: .base, from: .MARS_OPPORTUNITY, queryParameters: parameters, apiKey: queryParameters.authenticated)
            case .spirit:
                url = URLComposer.buildURL(baseUrl: .base, from: .MARS_SPIRIT, queryParameters: parameters, apiKey: queryParameters.authenticated)
            }
            print(url)
